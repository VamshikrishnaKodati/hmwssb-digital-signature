...
export default function MaterialEstimatePrint({ estimate, items }) {
      ...
      <table className="print-table">
        ...
        <tfoot>
          <tr className="total-row">
            <td colSpan="9" className="total-label">Part-I : Cost of Materials Total</td>
            <td className="total-amount">{fmt(total)}</td>
          </tr>
        </tfoot>
      </table>

      {estimate.Abstract?.MaterialTotalInWords && (
        <p className="words-row">(Rupees {estimate.Abstract.MaterialTotalInWords} Only)</p>
      )}
    </div>
  )
}
