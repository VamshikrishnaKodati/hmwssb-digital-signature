  ...
  return (
    <div className="min-w-0 space-y-6" data-testid="dashboard-shell">
      ...
      <QuickActions actions={[
        ...
        { label: 'Approvals', desc: 'Review DOP submitted estimates', icon: CheckCircle, to: qs({ status: 'DOP_Approved', assignedTo: 'me' }), tileBg: 'bg-green-50 hover:bg-green-100', border: 'border-green-200 border-l-green-600', iconBg: 'bg-green-600', btnBg: 'bg-green-600 group-hover:bg-green-700', focusRing: 'focus-visible:ring-green-400' },
      ...
      <QueueTable
        title="My Queue — Pending ED Approval"
        ...
        columns={[
          ...
          { key: 'action', label: 'Action', align: 'right', render: e => (
            <Link to={`/estimates/${e.EstimateID}`} onClick={ev => ev.stopPropagation()} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium bg-blue-600 text-white hover:bg-blue-700 transition-colors">
              Review
            </Link>
          )},
        ]}
      ...
      <WorkflowPosition
        ...
        stages={[
          { label: 'DOP', count: queues.submittedCreated || 0, color: '#D97706', bg: 'bg-amber-50', border: 'border-amber-200' },
          { label: 'ED', count: pendingReview, color: '#4338CA', bg: 'bg-indigo-50', border: 'border-indigo-200', to: qs({ status: 'DOP_Approved', assignedTo: 'me' }) },
          { label: 'MD', count: waitingForMD, color: '#9333EA', bg: 'bg-purple-50', border: 'border-purple-200' },
