    ...
    return (
      <div className="min-w-0">
        <div className="flex items-center gap-2 mb-5">
          <button onClick={() => navigate(-1)} className="p-1 rounded text-[#475569] hover:bg-[#F1F5F9]">
    ...
    <div className="min-w-0">
      ...
      <div className="ec-card mb-6">
        ...
        <div className="ec-card-body">
          <p className="text-xs text-[#475569] mb-4">Generate professional HMWSSB estimate documents as a single file.</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <button onClick={() => exportFile('pdf')} disabled={!!exporting}
              className="flex items-center gap-4 p-5 rounded-xl border-2 border-[#2563EB] bg-[#2563EB] text-white hover:bg-[#1D4ED8] transition-all text-left shadow-sm">
            ...
            <button onClick={() => exportFile('excel')} disabled={!!exporting}
              ...
              <span>
                ...
                <span className="block text-xs text-[#475569] mt-0.5">Workbook with all estimate sheets</span>
              </span>
            </button>
          </div>
        </div>
      ...
      {/* HMWSSB Header */}
      <div className="bg-white border border-[#CBD5E1] rounded-lg p-6 mb-5 text-center">
        <h2 className="text-lg font-bold text-[#2563EB]">HYDERABAD METROPOLITAN WATER SUPPLY & SEWERAGE BOARD</h2>
        <p className="text-sm text-[#475569] mt-1">Works Management System — Abstract of Estimate</p>
      ...
      {civilItems.length > 0 && (
        <div className="ec-card mb-5">
          <div className="ec-card-header bg-[#F8FAFC] border-b-2 border-[#2563EB]">
            <span className="ec-card-title">Part I — Estimate for Civil Work</span>
            <span className="text-xs text-[#475569]">{civilItems.length} items</span>
          </div>
          <div className="overflow-x-auto">{renderItemsTable(civilItems)}</div>
          <div className="px-4 py-3 border-t-2 border-[#CBD5E1] bg-[#F8FAFC] flex justify-end">
            <div className="text-right">
              <span className="text-xs text-[#475569]">Civil Work Total</span>
              <p className="text-base font-bold text-[#2563EB]">{fmt(a.CivilTotal)}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {materialItems.length > 0 && (
        <div className="ec-card mb-5">
          <div className="ec-card-header bg-[#F8FAFC] border-b-2 border-[#2563EB]">
            ...
            <span className="text-xs text-[#475569]">{materialItems.length} items</span>
          </div>
          <div className="overflow-x-auto">{renderItemsTable(materialItems)}</div>
          <div className="px-4 py-3 border-t-2 border-[#CBD5E1] bg-[#F8FAFC] flex justify-end">
            <div className="text-right">
              <span className="text-xs text-[#475569]">Material Total</span>
              <p className="text-base font-bold text-[#2563EB]">{fmt(a.MaterialTotal)}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="ec-card mb-5 border-2 border-[#2563EB]">
        <div className="ec-card-header bg-[#2563EB] text-white">
        ...
        <div className="ec-card-body">
          <table className="w-full max-w-lg mx-auto text-sm">
            <tbody>
              ...
              <tr className="border-b border-[#CBD5E1]">
                <td className="py-2 pl-8 text-[#475569]">Cost of Material</td>
                <td className="py-2 text-right font-medium">{fmt(a.MaterialTotal)}</td>
              </tr>
              <tr className="border-b border-[#CBD5E1]">
                <td className="py-2 pl-8 text-[#475569]">Cost of Civil Work</td>
                <td className="py-2 text-right font-medium">{fmt(a.CivilTotal)}</td>
              </tr>
              <tr className="border-b-2 border-[#2563EB] bg-[#F8FAFC]">
                <td className="py-3 pl-8 font-bold text-[#2563EB]">Cost of Estimate (Civil + Material)</td>
                <td className="py-3 text-right font-bold text-[#2563EB]">{fmt(a.CostOfEstimate)}</td>
              </tr>
              ...
              <tr className="border-b border-[#CBD5E1]">
                <td className="py-2 pl-8 text-[#475569]">GST @ {estimate.GSTPercent}%</td>
                <td className="py-2 text-right font-medium">{fmt(a.GST)}</td>
              </tr>
              <tr className="border-b border-[#CBD5E1]">
                <td className="py-2 pl-8 text-[#475569]">Additional Items</td>
              ...
              <tr className="border-b border-[#CBD5E1]">
                <td className="py-2 pl-8 text-[#475569]">LS unforeseen items and rounding off</td>
          ...
          <div className="mt-6 text-center text-xs text-[#475569] border-t border-[#CBD5E1] pt-4">
